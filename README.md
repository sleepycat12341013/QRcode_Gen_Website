...
| デザインテンプレート | 業種別プリセット |
| QR コード履歴管理 | 作成済み QR の保存・編集・削除 |
| アクセス解析 | スキャン数・地域・デバイス統計 |
| フォルダ管理・チーム共有 | 複数メンバーでの運用 |
| 短縮 URL 連携 | QR に埋め込む URL を後から変更可能 |
| API 提供 | 外部システムからの QR 生成 |
| 動画 QR 対応 | YouTube など動画 URL に特化したテンプレート |
