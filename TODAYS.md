...
### SVG ダウンロード機能（Pro ゲート付き）

- **PNG/SVG 2ボタン構成に変更**（index.html / text-qr.html / qr-email.html / qr-map.html）
- **SVG ボタンに Pro バッジ追加**（鍵アイコン + "Pro" ピル、赤半透明）
- クリックで pricing.html へ遷移（認証実装後に差し替え）
- `.btn-secondary` / `.download-row` / `.pro-badge` CSS を styles.css に追加

### UI 修正

- **新規登録/ログインボタン常時表示**  
  レスポンシブ時に右サイドバーに入る `btn-login--mobile` が白テキスト・白ボーダーで見えなかった問題を修正  
  → 赤テキスト・赤ボーダーのアウトラインスタイルに変更、全画面幅で常時表示
...
明日の5時間でSVGダウンロードまで終わる前提で：

① ロゴ・アイコン埋め込みQR実装（Proプランの目玉機能）

Canvas API でQRコード中央に画像を重ねる
画像アップロードUI追加
Claudeに頼めば2〜3時間
② Stripe Payment Links 作成（バックエンド不要・30分）

stripe.com でアカウント作成
Pro（¥580/月）と Business（¥1,980/月）の定期課金リンクを作成
pricing.html のボタンに貼るだけ
③ tokushoho.html の空欄を記入

有料プランを表示する前に法律上必須
住所はバーチャルオフィス契約まで個人の住所でも可（非公開にしたければ先にバーチャルオフィス契約）
この3つが終わるとProプランを正式リリースできる状態になります。
